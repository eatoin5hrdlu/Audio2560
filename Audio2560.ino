...
ISR(TIMER1_COMPA_vect) {
    if (sample >= sounddata_length) {
        ...
        else // Ramp down to zero to reduce click
		_OCR_ = sounddata_length + lastSample - sample;
    }
    else {
	if (hilo) {
		ldata = pgm_read_byte(&sounddata_data[sample++]);
		if (ldata == 0) { newvalue = 128; dir = true; }
		incr = 	ldata >> 4;
		if (incr == 0) dir = !dir;
		if (dir)
...
void startPlayback()
{
    ...
    // Macros change the following code to setup the right pin (11 or 3)
    TCCR2A = (TCCR2A | _BV(COMON1)) & ~_BV(COMON0);
    TCCR2A &= ~(_BV(COMOFF1) | _BV(COMOFF0));
    TCCR2B = (TCCR2B & ~(_BV(CS12) | _BV(CS11))) | _BV(CS10);
    _OCR_ = pgm_read_byte(&sounddata_data[0]);
